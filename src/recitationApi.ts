...
export type VerseTiming = { verseKey: string; startMs: number; endMs: number; segments: AudioSegment[] }
export type ChapterAudio = { audioUrl: string; timestamps: VerseTiming[] }
...
export async function fetchChapterAudio(reciterId: number, chapterNumber: number, credentials: QuranApiCredentials, includeSegments = true): Promise<ChapterAudio> {
  const data = await request<{ audio_file?: { audio_url?: string; timestamps?: Array<{ verse_key: string; timestamp_from: number; timestamp_to: number; segments?: AudioSegment[] }> }; audio_url?: string; timestamps?: Array<{ verse_key: string; timestamp_from: number; timestamp_to: number; segments?: AudioSegment[] }> }>(`/chapter_reciters/${reciterId}/audio_files/${chapterNumber}?segments=${includeSegments}`, credentials)
  const source = data.audio_file ?? data
  return { audioUrl: source.audio_url ?? '', timestamps: (source.timestamps ?? []).map(t => ({ verseKey: t.verse_key, startMs: t.timestamp_from, endMs: t.timestamp_to, segments: t.segments ?? [] })) }
}
...
export function findActiveTiming(timings: VerseTiming[], timeMs: number) {
  ...
  return { verseKey: verse.verseKey, segmentIndex, wordIndex: segmentIndex < 0 ? null : verse.segments[segmentIndex][0] }
}
