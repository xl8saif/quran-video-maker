...
export interface ScrolledRenderOptions {
  scrollTop?: number
  scale?: number
  showFinger?: boolean
...
export function renderScrolledQuranPage(
  ctx: CanvasRenderingContext2D,
  pageImage: HTMLImageElement,
  page: QuranPage | undefined,
  time: number,
  options: ScrolledRenderOptions = {},
) {
  if (!pageImage.naturalWidth || !pageImage.naturalHeight) return
  const canvas = ctx.canvas
  const scale = options.scale ?? Math.min(canvas.width / page.width, canvas.height / page.height)
  ...
  ctx.save()
  ctx.translate(offsetX, -scrollTop)
  ctx.scale(scale, scale)
  ctx.drawImage(pageImage, 0, 0, page.width, page.height)

  const focus = getSyncedFocus(page, time)
  if (focus.word) {
    const line = page.lines.find(item => item.number === focus.word?.line)
    if (line) drawActiveLine(ctx, 0, line.y, page.width, line.height)
    drawActiveWord(ctx, focus.word)
    if (options.showFinger !== false) drawFinger(ctx, focus.finger, options.fingerImage)
  }
  ctx.restore()
}
