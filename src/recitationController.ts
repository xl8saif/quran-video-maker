...
export function syncAudioElement(audio:HTMLAudioElement,timing:{timestamps:Parameters<typeof findActiveTiming>[0]},onSync:(state:RecitationSyncState)=>void){
 const handleTime=()=>{const timeMs=audio.currentTime*1000;const active=findActiveTiming(timing.timestamps,timeMs);onSync({timeMs,durationMs:Math.max(audio.duration*1000||0,timingDuration(timing.timestamps)),verseKey:active?.verseKey??'',wordIndex:active?.wordIndex??0})}
...
export async function fetchChapterTiming(endpoint:string,chapterNumber:number,reciterId:number,accessToken:string,clientId:string){
...
export function createRecitationController(credentials:QuranApiCredentials){
 ...
 return {subscribe(listener:(state:RecitationState)=>void){listeners.add(listener);listener({...state});return()=>listeners.delete(listener)},async loadChapter(reciterId:number,chapterNumber:number){
  ...
  emit();return {...state}
 },updateTime(timeMs:number){if(!state.audio)return;const active=findActiveTiming(state.audio.timestamps,timeMs);state={...state,activeVerse:active?.verseKey,activeWordIndex:active?.wordIndex??0};emit()},getState:()=>({...state}),destroy(){listeners.clear()}}
}
