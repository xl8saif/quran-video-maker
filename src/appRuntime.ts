...
export function createAppRuntime():AppRuntime{
 ...
 const startExport=(nextOptions:ExportPanelOptions)=>{stopTimer();stopTimeoutTimer();cleanupStream();const thisRun=++runId;if(recorder&&recorder.state!=='inactive'){recorder.onstop=null;recorder.stop()}recorder=undefined;const speedControl=ensureExportSpeedControl();const selectedSpeed=Number(speedControl?.value);options={...nextOptions,playbackSpeed:(Number.isFinite(selectedSpeed)&&selectedSpeed>0?selectedSpeed:nextOptions.playbackSpeed??1) as ExportPanelOptions['playbackSpeed']};const speed=normalizeExportSpeed(options.playbackSpeed);if(!media){state={status:'error',progress:0,elapsed:0,duration:0,error:'Export canvas is not available.'};emit();return}const audio=resolveAudio();const sizes:Record<ExportPanelOptions['resolution'],[number,number]>={'720p':[1280,720],'1080p':[1920,1080],'1440p':[2560,1440],'2160p':[3840,2160]};const [width,height]=sizes[options.resolution];media.canvas.width=width;media.canvas.height=height;streamController=createCanvasAudioStreamController(media.canvas,audio,options.fps);stream=streamController.stream;const mimeType=MediaRecorder.isTypeSupported('video/webm;codecs=vp9,opus')?'video/webm;codecs=vp9,opus':'video/webm';try{recorder=new MediaRecorder(stream,{mimeType})}catch(error){cleanupStream();state={status:'error',progress:0,elapsed:0,duration:0,error:error instanceof Error?error.message:'MediaRecorder is unavailable.'};emit();return}chunks=[];const mediaDuration=audio?.duration&&Number.isFinite(audio.duration)&&audio.duration>0?audio.duration*1000:0;const duration=getExportDurationMs(mediaDuration,speed);const started=performance.now();let ended=false;const stopRecording=()=>{if(ended||thisRun!==runId||recorder?.state==='inactive')return;ended=true;audio?.pause();recorder?.stop()};const handleEnded=()=>stopRecording();audio?.addEventListener('ended',handleEnded,{once:true});const removeEndedListener=()=>audio?.removeEventListener('ended',handleEnded);recorder.ondataavailable=event=>{if(thisRun===runId&&event.data.size)chunks.push(event.data)};recorder.onerror=event=>{if(thisRun!==runId)return;removeEndedListener();stopTimer();stopTimeoutTimer();state={...state,status:'error',error:event.error?.message??'MediaRecorder failed.'};cleanupStream();recorder=undefined;emit()};recorder.onstop=()=>{const current=thisRun===runId;removeEndedListener();stopTimer();stopTimeoutTimer();audio?.pause();cleanupStream();if(!current){if(recorder?.state==='inactive')recorder=undefined;return}const actualElapsed=Math.max(0,performance.now()-started);const finalDuration=duration||actualElapsed;const blobUrl=URL.createObjectURL(new Blob(chunks,{type:mimeType}));state={...state,status:'ready',progress:100,elapsed:finalDuration,duration:finalDuration,blobUrl};recorder=undefined;emit()};state={status:'recording',progress:0,elapsed:0,duration};emit();try{recorder.start(250)}catch(error){removeEndedListener();state={status:'error',progress:0,elapsed:0,duration,error:error instanceof Error?error.message:'Unable to start recording.'};cleanupStream();recorder=undefined;emit();return}if(audio){audio.currentTime=0;audio.playbackRate=speed;void audio.play()};timer=window.setInterval(()=>{if(thisRun!==runId)return;const elapsed=duration?Math.min(duration,performance.now()-started):Math.max(0,performance.now()-started);state={...state,elapsed,progress:duration?elapsed/duration*100:0};emit()},100);const safetyDuration=duration>0?duration+2000:60*60*1000;stopTimeout=window.setTimeout(stopRecording,safetyDuration)}
