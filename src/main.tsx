  ...
  <main className="workspace">
   <aside className="sidebar left-panel">
    ...
    <section className="panel-section background-section"><div className="section-title">{t.background}</div>
      <div className="media-grid">{BUILTIN_BACKGROUND_SOURCES.map(media=><button type="button" className={`media-card ${background?.id===media.id?'selected':''}`} key={media.id} onClick={()=>selectBuiltin(media)}>{media.kind==='video'?<Video size={14}/>:<ImageIcon size={14}/>}<span>{media.name}</span><small>{media.license}</small></button>)}</div>
