...
function App(){
 ...
 const handleLogo=(file:File)=>{const url=URL.createObjectURL(file);setLogoUrl(previous=>{if(previous)URL.revokeObjectURL(previous);return url})}

 return <div className="app-shell" dir={dir}>
  <audio ref={audioRef} />
  <header className="topbar"><div><div className="brand">Waraq Quran Reel Maker</div><div className="subtitle">{t.sourceNote}</div></div><div className="top-actions"><select className="ui-language" aria-label={t.language} value={uiLang} onChange={e=>setUiLang(e.target.value as UILanguage)}><option value="en">English</option><option value="ur">اردو</option><option value="ar">العربية</option></select></div></header>
  <main className="workspace">
   <aside className="sidebar left-panel">
    <section className="panel-section"><div className="section-title"><BookOpen size={16}/>{t.quran}</div>
      <label>{t.surah}</label><select value={surahNumber} onChange={e=>{setSurahNumber(Number(e.target.value));setSearchQuery('')}}>{chapterOptions.map(s=><option key={s.id} value={s.id}>{s.id}. {s.name_simple} — {s.name_arabic||''}</option>)}</select>
      ...
      <label className="search-box"><Search size={16}/><input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={t.searchPage}/>{searchQuery&&<button onClick={()=>setSearchQuery('')} aria-label={t.close}><X size={14}/></button>}</label>
    </section>
    <section className="panel-section"><div className="section-title"><Languages size={16}/>{t.translations}</div>
      <button className="wide-button" onClick={()=>setShowLibrary(true)}>{t.library}</button>
      <div className="selected-source-list">{externalTranslations.map(source=><div className="source-chip" key={source.key}><span>{source.language.toUpperCase()} · {source.title}</span><button onClick={()=>setSelectedSources(list=>list.filter(id=>id!==source.key))} aria-label={t.remove}><X size={13}/></button></div>)}</div>
  ...
  {showLibrary&&<div className="modal-backdrop" onClick={()=>setShowLibrary(false)}><section className="translation-library" onClick={e=>e.stopPropagation()} dir={dir}><header><div><strong>{t.external}</strong><small>{libraryLoading?'Loading…':`${filteredSources.length} sources`}</small></div><button onClick={()=>setShowLibrary(false)} aria-label={t.close}><X/></button></header><label className="search-box"><Search size={16}/><input value={translationSearch} onChange={e=>setTranslationSearch(e.target.value)} placeholder={t.searchTranslations}/></label><div className="translation-list">{filteredSources.map(([id,source])=>{const selected=selectedSources.includes(id);return <div className={`translation-item ${selected?'selected':''}`} key={id}><div><strong>{source.title}</strong><small>{source.language_iso_code.toUpperCase()} · v{source.version}</small><p>{source.description}</p></div><button className={selected?'ghost':'primary'} onClick={()=>setSelectedSources(list=>selected?list.filter(x=>x!==id):[...list,id])}>{selected?t.remove:t.add}</button></div>})}{!filteredSources.length&&<div className="empty-data"><h3>{t.noSources}</h3></div>}</div><footer>{t.attribution}</footer></section></div>}
 </div>
}
