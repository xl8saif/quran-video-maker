...
function App(){
 ...
 const selectBuiltin=(media:BackgroundMedia)=>setBackground(media)
 ...
 return <div className={`app-shell theme-${uiLang}`} dir={dir}>
  <audio ref={audioRef} />
  <header className="topbar">
   <div className="brand-logo glass-logo"><img src={DEFAULT_LOGO} alt="Waraq" /></div>
   <div className="title-block"><h1>Waraq Quran Reel Maker</h1><div className="subtitle">{t.sourceNote}</div><nav className="language-switcher" aria-label={t.language}>{(['en','ar','ur'] as UILanguage[]).map(lang=><button key={lang} className={uiLang===lang?'active':''} onClick={()=>setUiLang(lang)}>{lang==='en'?'En':lang==='ar'?'Ar':'Ur'}</button>)}</nav></div>
  ...
  <main className="workspace">
   <aside className="sidebar left-panel">
    ...
    <section className="panel-section background-section"><div className="section-title">{t.background}</div>
      <div className="media-grid">{BUILTIN_BACKGROUND_SOURCES.map(media=><button className={`media-card ${background?.id===media.id?'selected':''}`} key={media.id} onClick={()=>selectBuiltin(media)}>{media.kind==='video'?<Video size={14}/>:<ImageIcon size={14}/>}<span>{media.name}</span><small>{media.license}</small></button>)}</div>
    ...
    <section className="panel-section"><div className="section-title">{t.logo}</div>
      <div className="logo-choice-row"><button className={logoChoice==='waraq'?'selected':''} onClick={()=>chooseLogo('waraq')}>{t.defaultLogo}</button><button className={logoChoice==='cloudtrans'?'selected':''} onClick={()=>chooseLogo('cloudtrans')}>{t.cloudLogo}</button></div>
