...
function countPages(quran:Map<number,string[]>,throughSurah=114){let pages=0;for(const [s,verses] of quran){if(s>=throughSurah)break;pages+=Math.max(1,Math.ceil(verses.slice(1).filter(Boolean).length/7))}return pages}
...
export async function fetchChapterPages(chapterNumber:number,style:'hafs-arabic-naskh'|'indo-pak-muhammadi',_config?:MushafApiConfig):Promise<PagesLookupResponse>{
 const quran=await getQuran(style);const verses=quran.get(chapterNumber)||[];const ayahs=verses.slice(1).filter(Boolean).length;const firstPage=countPages(quran,chapterNumber)+1;const pageCount=Math.max(1,Math.ceil(ayahs/7));const pages:Record<string,MushafPageBoundary>={}
 for(let index=0;index<pageCount;index++){const first=index*7+1,last=Math.min(ayahs,first+6),page=firstPage+index;pages[String(page)]={from:`${chapterNumber}:${first}`,to:`${chapterNumber}:${last}`,first_verse_key:`${chapterNumber}:${first}`,last_verse_key:`${chapterNumber}:${last}`}}
...
export async function fetchPage(pageNumber:number,style:'hafs-arabic-naskh'|'indo-pak-muhammadi',_config?:MushafApiConfig,_translationIds:number[]=[]):Promise<{verses:ApiVerse[]}>{
 const quran=await getQuran(style);const all:ApiVerse[]=[];let globalVerse=0;const first=(Math.max(1,pageNumber)-1)*7+1,end=first+6
 for(const [surah,verses] of quran.entries()) for(let ayah=1;ayah<verses.length;ayah++){if(!verses[ayah])continue;globalVerse++;if(globalVerse<first)continue;if(globalVerse>end)return {verses:all};const verseKey=`${surah}:${ayah}`,lineNumber=((globalVerse-first)%7)+1,words=makeWords(verses[ayah],verseKey,pageNumber,lineNumber,style==='indo-pak-muhammadi');all.push({verse_key:verseKey,verse_number:ayah,page_number:pageNumber,text_uthmani:style==='indo-pak-muhammadi'?undefined:verses[ayah],text_indopak:style==='indo-pak-muhammadi'?verses[ayah]:undefined,text_qpc_hafs:style==='indo-pak-muhammadi'?undefined:verses[ayah],words})}
 return {verses:all}
}
