...
export function LiveRecitationControls({ chapterNumber, onSync, onStatus }: Props) {
  ...
  React.useEffect(() => {
    ...
    void fetchChapterRecitations().then(result => {
      ...
      if (result.recitations.length && !result.recitations.some(item => item.id === reciterId)) {
        setReciterId(result.recitations[0].id)
      }
    }).catch(errorValue => {
  ...
  return <div className="live-recitation-controls">
    <div className="recitation-row">
      <button className="primary" type="button" onClick={togglePlayback} disabled={!audio || loadingAudio} aria-label={playing ? 'Pause recitation' : 'Play recitation'}>
