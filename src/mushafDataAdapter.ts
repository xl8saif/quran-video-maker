...
export type MushafAdapterOptions = {
  ...
  accessToken?: string
  clientId?: string
  translationByVerse?: Record<string, { en?: string; ur?: string; ar?: string }>
}

export function getMushafId(styleId: keyof typeof MUSHAF_ENGINE_CONFIG) {
...
export function normalizeMushafPage(
  ...
  return payload.verses.map((verse) => ({
    ...
    translations: {
      en: translations[verse.verse_key]?.en ?? '',
      ur: translations[verse.verse_key]?.ur ?? '',
      ar: translations[verse.verse_key]?.ar ?? '',
    },
    words: (verse.words ?? []).map((word): QuranWord => ({
      text: useIndoPak ? (word.text_indopak ?? '') : (word.text_qpc_hafs ?? word.text_uthmani ?? ''),
      start: 0,
      end: 0,
...
export async function fetchMushafPage(page: number, options: MushafAdapterOptions): Promise<MushafPagePayload> {
  ...
  if (options.accessToken) headers['x-auth-token'] = options.accessToken
  if (options.clientId) headers['x-client-id'] = options.clientId

  const response = await fetch(url, { headers })
  if (!response.ok) throw new Error(`Mushaf request failed: ${response.status}`)
  return response.json() as Promise<MushafPagePayload>
}
