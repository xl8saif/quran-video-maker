...
function LiveMushafPreviewInner({ styleId, page, accessToken = '', clientId = '', chapterNumber, activeVerse, activeWordIndex = 0, highlight, showFinger = true, autoScroll = true, scrollSpeed = 50, onStatus, exportCanvasRef, exportBackground, exportLogo, exportTranslations = [], translationLanguages = [], externalTranslations = [], searchQuery = '' }: Props) {
  ...
  React.useEffect(() => {
    ...
    void getCachedQuranPage(key).then(cached => { if (cancelled || !cached?.length) return; setVerses(cached); setLoading(false); onStatus?.(`Cached Mushaf page ${page} loaded`) })
    fetchPage(page, styleId, { accessToken, clientId }, translationIds).then(data => { if (cancelled) return; const nextVerses = data.verses || []; setVerses(nextVerses); setError(''); setLoading(false); void cacheQuranPage(key, nextVerses); onStatus?.(`Mushaf page ${page} loaded`) }).catch(e => { if (cancelled) return; setLoading(false); setVerses(current => { if (current.length) return current; setError(e instanceof Error ? e.message : 'Unable to load Mushaf page'); return [] }); onStatus?.('Mushaf data unavailable; retrying from local cache') })
  ...
  const lines = React.useMemo(() => { const map = new Map<number, { verseKey: string; position: number; text: string }[]>(); filteredVerses.flatMap(v => v.words || []).forEach(w => { const text = styleId === 'indo-pak-muhammadi' ? (w.text_indopak || '') : (w.text_qpc_hafs || w.text_uthmani || ''); if (!text) return; const line = map.get(w.line_number) || []; line.push({ verseKey: w.verse_key, position: w.position, text }); map.set(w.line_number, line) }); return [...map.entries()].sort((a,b) => a[0]-b[0]).map(([line, words]) => [line, words.sort((a,b) => a.position-b.position)] as const) }, [filteredVerses, styleId])
  ...
  const drawMushaf = React.useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => { ctx.fillStyle = '#17120c'; ctx.textAlign = 'right'; ctx.direction = 'rtl'; const lineHeight = Math.max(34, height / Math.max(lines.length + 2, 10)); const startY = Math.max(55, (height - lineHeight * lines.length) / 2 + lineHeight); lines.forEach(([, words], index) => { const isActive = effectiveActiveVerse ? words.some(w => w.verseKey === effectiveActiveVerse) : false; let x = width - 70; const y = startY + index * lineHeight; ctx.font = `${Math.max(24, Math.min(52, width / 25))}px '${quranFont(styleId)}'`; words.forEach(word => { const active = Boolean(effectiveActiveVerse && word.verseKey === effectiveActiveVerse && activeWord && word.position === activeWord.position); ctx.fillStyle = active ? highlight : isActive ? '#806b45' : '#17120c'; ctx.fillText(word.text, x, y); x -= ctx.measureText(word.text + ' ').width }) }) }, [lines, effectiveActiveVerse, activeWord, highlight, styleId])

  React.useEffect(() => { const canvas = exportCanvasRef?.current; if (!canvas || !lines.length) return; let cancelled = false; compositorRef.current?.destroy(); compositorRef.current = null; const prepare = async () => { try { await document.fonts.load(`40px '${quranFont(styleId)}'`) } catch {} if (cancelled) return; const compositor = await createExportCompositor({ canvas, width: canvas.width || 1280, height: canvas.height || 720, background: exportBackground, logo: exportLogo, translations: exportTranslations, drawMushaf }); if (cancelled) compositor.destroy(); else compositorRef.current = compositor }; void prepare().catch(errorValue => { if (!cancelled) onStatus?.(errorValue instanceof Error ? errorValue.message : 'Export compositor failed') }); return () => { cancelled = true; compositorRef.current?.destroy(); compositorRef.current = null } }, [exportCanvasRef, exportBackground, exportLogo, exportTranslations, drawMushaf, lines.length, onStatus, styleId])
  ...
  const liveLogo = exportLogo?.url ? <img className="live-player-logo" src={exportLogo.url} alt="" aria-hidden="true" /> : null

  if (loading && !verses.length) return <div className="live-mushaf-state"><strong>Loading bundled Mushaf page {page}</strong><small>Preparing local Quran text and page data…</small>{liveLogo}{exportCanvas}</div>
  if (error && !verses.length) return <div className="live-mushaf-state error"><strong>Mushaf page unavailable</strong><span>{error}</span><small>The app will use locally cached Quran pages when available.</small>{liveLogo}{exportCanvas}</div>
  if (!lines.length) return <div className="live-mushaf-state">{searchQuery ? 'No matching verses on this page.' : 'No page data returned.'}{liveLogo}{exportCanvas}</div>
  return <div className="quran-live-page" dir="rtl" translate="no" ref={pageRef}>
    ...
    <div className="live-translations" translate="no">{translationRows.map(({verse, translations}) => <div key={verse.verse_key} className="live-translation-verse"><span className="live-translation-key">{verse.verse_key}</span>{translations.map(item => <div key={`${item.resource_id}-${item.language}`} className={`live-translation live-translation-${item.language}`} dir={item.language === 'en' ? 'ltr' : 'rtl'}><span className="live-translation-label">{languageLabel(item.language)}</span><span dangerouslySetInnerHTML={{ __html: item.text }} /></div>)}</div>)}{externalTranslations.map(source => filteredVerses.filter(verse => source.entries[verse.verse_key]).map(verse => <div key={`${source.key}-${verse.verse_key}`} className="live-translation-verse"><span className="live-translation-key">{verse.verse_key}</span><div className={`live-translation live-translation-${source.language}`} dir={source.language === 'en' ? 'ltr' : 'rtl'}><span className="live-translation-label">{languageLabel(source.language)} · {source.title}</span><span>{source.entries[verse.verse_key]}</span></div></div>))}</div>
    {exportCanvas}
  </div>
}

function sameValue<T>(a: T | undefined, b: T | undefined) { return Object.is(a, b) || JSON.stringify(a) === JSON.stringify(b) }
const liveMushafPropsEqual = (previous: Props, next: Props) => previous.styleId === next.styleId && previous.page === next.page && previous.accessToken === next.accessToken && previous.clientId === next.clientId && previous.chapterNumber === next.chapterNumber && previous.activeVerse === next.activeVerse && previous.activeWordIndex === next.activeWordIndex && previous.highlight === next.highlight && previous.showFinger === next.showFinger && previous.autoScroll === next.autoScroll && previous.scrollSpeed === next.scrollSpeed && previous.exportCanvasRef === next.exportCanvasRef && previous.onStatus === next.onStatus && sameValue(previous.exportBackground, next.exportBackground) && sameValue(previous.exportLogo, next.exportLogo) && sameValue(previous.exportTranslations, next.exportTranslations) && sameValue(previous.translationLanguages, next.translationLanguages) && sameValue(previous.externalTranslations, next.externalTranslations) && previous.searchQuery === next.searchQuery
