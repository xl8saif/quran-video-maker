...
export function LiveMushafPreview({ styleId, page, accessToken = '', clientId = '', activeVerse, activeWordIndex = 0, highlight, showFinger = true, autoScroll = true, scrollSpeed = 50, onStatus, exportCanvasRef }: Props) {
  ...
  const lines = React.useMemo(() => {
    ...
    verses.flatMap(v => v.words || []).forEach(w => {
      ...
      line.push({ verseKey: w.verse_key, position: w.position, text }); map.set(w.line_number, line)
    })
  ...
  React.useEffect(() => {
    if (!autoScroll || !activeWordRef.current || !pageRef.current) return
    const word = activeWordRef.current, pageEl = pageRef.current
    const pageRect = pageEl.getBoundingClientRect(), wordRect = word.getBoundingClientRect()
    const delta = wordRect.top - (pageRect.top + pageRect.height * 0.48)
  ...
  React.useLayoutEffect(() => {
    ...
    const update = () => { const wr = word.getBoundingClientRect(), pr = pageEl.getBoundingClientRect(); setFingerStyle({ opacity: 1, left: `${wr.left - pr.left + wr.width / 2}px`, top: `${wr.bottom - pr.top + 6}px`, transitionDuration: `${Math.max(100, 500 - scrollSpeed * 4)}ms` }) }
    update(); window.addEventListener('resize', update); pageEl.addEventListener('scroll', update, { passive: true })
  ...
  return <div className="quran-live-page" dir="rtl" ref={pageRef}>
    ...
    {lines.map(([lineNumber, words]) => { const active = activeVerse ? words.some(w => w.verseKey === activeVerse) : false; return <div key={lineNumber} className={`live-quran-line ${active ? 'active-line' : ''}`} style={active ? ({ '--highlight': highlight } as React.CSSProperties) : undefined}>{words.map(w => { const isActiveWord = Boolean(activeVerse && w.verseKey === activeVerse && activeWord && w.position === activeWord.position); return <span ref={isActiveWord ? activeWordRef : null} key={`${w.verseKey}-${w.position}`} className={isActiveWord ? 'active-live-word' : w.verseKey === activeVerse ? 'active-live-word-soft' : ''}>{w.text} </span> })}</div> })}
