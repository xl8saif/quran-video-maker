...
const SOCIAL_TARGETS=[
 {id:'youtube',label:'YouTube',url:'https://studio.youtube.com/',icon:'▶'},
 {id:'tiktok',label:'TikTok',url:'https://www.tiktok.com/tiktokstudio/upload',icon:'♪'},
 {id:'facebook',label:'Facebook',url:'https://www.facebook.com/creatorstudio/',icon:'f'},
 {id:'instagram',label:'Instagram',url:'https://www.instagram.com/',icon:'◎'},
 {id:'bilibili',label:'BiliBili',url:'https://member.bilibili.com/platform/upload/video/frame',icon:'B'},
] as const
...
export function createExportPanel(bindings:ExportPanelBindings):HTMLElement {
 ...
 select('Video dimensions',bindings.options.resolution,[{value:'youtube-landscape',label:'YouTube 16:9 — 1920×1080'},{value:'youtube-shorts',label:'YouTube Shorts 9:16 — 1080×1920'},{value:'square',label:'Square 1:1 — 1080×1080'},{value:'720p',label:'1280×720'},{value:'1080p',label:'1920×1080'},{value:'1440p',label:'2560×1440'},{value:'2160p',label:'4K — 3840×2160'}],v=>bindings.onOptionsChange?.({...bindings.options,resolution:v as ExportResolution}))
 ...
 SOCIAL_TARGETS.forEach(target=>{const button=document.createElement('button');button.type='button';button.className=`social-upload-button social-${target.id}`;button.title=`Open ${target.label} upload`;button.setAttribute('aria-label',`Open ${target.label} upload`);const icon=document.createElement('span');icon.className='social-upload-icon';icon.textContent=target.icon;const text=document.createElement('span');text.textContent=target.label;button.append(icon,text);button.addEventListener('click',async()=>{const tags=(bindings.options.seoTags??'').trim();if(tags&&navigator.clipboard?.writeText)await navigator.clipboard.writeText(tags);window.open(target.url,'_blank','noopener,noreferrer')});socialGrid.appendChild(button)}); social.appendChild(socialGrid)
 const seoHint=document.createElement('small');seoHint.className='export-seo-hint';seoHint.textContent='SEO tags are copied to your clipboard before the selected platform opens.';social.appendChild(seoHint); form.appendChild(social)
