...
export function normalizeFilename(name:string){const clean=name.trim().replace(/[^a-zA-Z0-9._-]+/g,'-').replace(/^-+|-+$/g,'');return `${clean||'quran-video'}.webm`}
export function createDefaultExportOptions():ExportPanelOptions{return {resolution:'1080p',fps:30,mushafStyle:'hafs-naskh',translationLanguage:'none',filename:'quran-video.webm'}}
...
export function createExportPanel(bindings:ExportPanelBindings):HTMLElement {
 ...
 select('Mushaf',bindings.options.mushafStyle,[{value:'hafs-naskh',label:'Hafs Arabic Naskh'},{value:'indo-pak-muhammadi',label:'Indo-Pak Muhammadi'}],v=>bindings.onOptionsChange?.({...bindings.options,mushafStyle:v}))
