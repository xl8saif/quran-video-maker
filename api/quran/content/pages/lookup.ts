...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const url = new URL(`${getQfApiBase(env)}/content/api/v4/pages/lookup`)
    ...
    if (!response.ok) return new Response(JSON.stringify({ error: 'Quran Foundation page lookup failed' }), { status: response.status === 403 ? 502 : response.status, headers: { 'Content-Type': 'application/json' } })
    ...
    console.error('Quran Foundation page lookup failed:', error instanceof Error ? error.message : 'unknown error')
    return new Response(JSON.stringify({ error: 'Unable to retrieve Quran Foundation page mapping' }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
}
